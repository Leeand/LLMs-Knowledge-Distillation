...
'''
def main():
    global experiments_group 
    experiments_group+=1
    dist.barrier()
    print(f"experiments_group_{experiments_group}")
    # wandb
    run = wandb.init(project=os.environ["WANDB_PROJECT"],group=f"{experiments_group}_experiments_group")
    ...
    if local_rank==0:
        ...
        wandb.run.name=f"{wandb.run.name}_{time.strftime('%Y%m%d-%H%M%S')}"
        wandb.run.save() 
    dist.barrier
    ...
    tune_ckpt_name = output_path+"/globalstep_309"
