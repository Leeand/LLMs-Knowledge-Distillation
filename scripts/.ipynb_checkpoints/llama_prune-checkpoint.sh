...
hyperparamaters_name='intermedia_k1_h1_t20_DDP' # update the name according the hyperparamaters
...
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python -m torch.distributed.launch --nproc_per_node 8 distill_training.py --teacher_model_config teacher_tune_log/$tune_ckpt_path/ --student_tune_model_config student_tune_log/$tune_ckpt_path --prune_model prune_log/$prune_ckpt_path/pytorch_model.bin --data_path yahma/alpaca-cleaned --output_dir tune_log/$hyperparamaters_name --wandb_project improvement_project --lora_r 8 --num_epochs 2 --learning_rate 1e-4 --batch_size 3 --kd_loss_weight 1 --hard_label_weight 1 --temperature 20 --kd_type original_kd
