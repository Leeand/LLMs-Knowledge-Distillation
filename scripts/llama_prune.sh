...
hyperparamaters_name='k1_h1_t1' # update the name according the hyperparamaters
...
# echo "[INFO] - The pruned model is at {prune_log/$prune_ckpt_path/pytorch_model.bin}, and the Student LORA weight is at {student_tune_log/$tune_ckpt_path}/"

echo "[START] - Start Student Distill"
CUDA_VISIBLE_DEVICES=0,1,2,3 python distill_training.py --teacher_model_config teacher_tune_log/$tune_ckpt_path/  --student_tune_model_config student_tune_log/$tune_ckpt_path --prune_model prune_log/$prune_ckpt_path/pytorch_model.bin --data_path yahma/alpaca-cleaned --output_dir tune_log/$hyperparamaters_name --wandb_project my-awesome-project --lora_r 8 --num_epochs 1 --learning_rate 1e-4 --batch_size 4 --kd_loss_weight 1 --hard_label_weight 1 --temperature 1
echo "[FINISH] - Finish Prune and Post-Training."
